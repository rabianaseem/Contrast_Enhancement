...
from albumentations.pytorch.transforms import torch as AT
...
def data_loader():
    ...
    testset_gt = torchvision.datasets.ImageFolder(root='./images_LR/Reference/Testing/',
        transform = transforms.Compose([transforms.Grayscale(num_output_channels=1),transforms.ToTensor()]))
    trainset_1_gt = torchvision.datasets.ImageFolder(root='./images_LR/Reference/Training1/',
        transform = transforms.Compose([transforms.Grayscale(num_output_channels=1),transforms.ToTensor()]))
    trainset_2_gt = torchvision.datasets.ImageFolder(root='./images_LR/Reference/Training2/',
        transform = transforms.Compose([transforms.Grayscale(num_output_channels=1),transforms.ToTensor()]))

    testset_inp = torchvision.datasets.ImageFolder(root='./images_LR/input/Testing/', 
        transform = transforms.Compose([transforms.Grayscale(num_output_channels=1),transforms.ToTensor()]))
    trainset_1_inp = torchvision.datasets.ImageFolder(root='./images_LR/input/Training1/',   
        transform = transforms.Compose([transforms.Grayscale(num_output_channels=1),transforms.ToTensor()]))
    trainset_2_inp = torchvision.datasets.ImageFolder(root='./images_LR/input/Training2/', 
        transform = transforms.Compose([transforms.Grayscale(num_output_channels=1),transforms.ToTensor()]))
...
def contentFunc(img):
    ...
    output = model(img)  # print(output)
    # output = vgg16.features[:3](img)  # print(output)
    return output	


def get_Perceptual_loss(fakeIm, realIm):
    # f_fake = self.contentFunc.forward(fakeIm)
    # f_real = self.contentFunc.forward(realIm)
    f_fake = contentFunc(fakeIm)
    f_real = contentFunc(realIm)
    f_real_no_grad = f_real.detach()    #loss = self.criterion(f_fake, f_real_no_grad)
    criterion = nn.MSELoss()
    c_loss = criterion(f_fake, f_real_no_grad).mean()
    return c_loss

# loss definition ends here
